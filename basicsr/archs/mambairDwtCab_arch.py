...
class CAB(nn.Module):
    def __init__(self,num_feat):
        ...
        self.c = nn.Sequential(
            ...
            nn.Conv2d(num_feat, num_feat, kernel_size=3,padding=1, groups=num_feat),
        )
    def forward(self,x):
        return self.c(x)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.catconv1 = nn.Conv2d(hidden_dim, hidden_dim, 1)
        self.self_attention = SS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
        self.wav_enhancement = WaveletConvBlock(hidden_dim,hidden_dim)
    ...
    def forward(self, input, x_size):
        ...
        # print("x1",x1.size())
        x2 = self.wav_enhancement(x)
        # print("x2",x2.size())
        # 通道软注意力
        out = torch.cat([x1, x2], dim=3)
        out = F.softmax(self.advavg(out), dim=3) * out
        out = out.permute(0, 3, 1, 2).contiguous()
        out1, out2 = torch.split(out, out.size(1) // 2, dim=1)
        # print("out1",out1.size())
        x = self.catconv1(out1 + out2)
...
class WaveletConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, wavelet='haar'):
        ...
        self.wconv1 = nn.Conv2d(in_channels=in_channels,out_channels = out_channels//2, kernel_size=1)
        # 定义静态卷积核权重参数，并初始化
        self.static_kernel = nn.Parameter(torch.ones(self.in_channels, self.out_channels, 3, 3))
        self.wavelet = wavelet

        self.pointwise_convs = nn.Conv2d(self.in_channels,self.out_channels,kernel_size=1)

        # 使用 pytorch_wavelets 进行 DWT 和 IDWT
        self.dwtWavelet = DWTForward(J=1, wave=self.wavelet).to('cuda')  # 只做一级小波分解
        self.idwt = DWTInverse(wave=self.wavelet).to('cuda')
        self.wconv12 = nn.Conv2d(in_channels=out_channels//2, out_channels=in_channels, kernel_size=1)

    def forward(self, x):
        x = x.permute(0, 3, 1, 2).contiguous()
        x = self.wconv1(x)
        # 进行小波变换，获取 DWT 系数
        dwt_coeffs = self.dwtWavelet(x)  # 返回 (approx, details)
        LL, details = dwt_coeffs
        # 拆分高频分量
        LH, HL, HH = details[0][:, :, 0, :, :], details[0][:, :, 1, :, :], details[0][:, :, 2, :, :]

        # 计算低频和高频卷积核
        static_weight = self.static_kernel.unsqueeze(0)
        low_freq_kernel = static_weight.mean(dim=(-1,-2), keepdim=True)
        high_freq_kernel = self.static_kernel - low_freq_kernel

        LL1 = F.conv2d(LL, low_freq_kernel.view(self.out_channels, self.in_channels, 1, 1), stride=1)
        LH1 = F.conv2d(LH, high_freq_kernel.view(self.out_channels, self.in_channels, 3, 3), stride=1, padding=1)
        HL1 = F.conv2d(HL, high_freq_kernel.view(self.out_channels, self.in_channels, 3, 3), stride=1, padding=1)
        HH1 = F.conv2d(HH, high_freq_kernel.view(self.out_channels, self.in_channels, 3, 3), stride=1, padding=1)

        LL2 = self.pointwise_convs(LL1)
        LH2 = self.pointwise_convs(LH1)
        HL2 = self.pointwise_convs(HL1)
        HH2 = self.pointwise_convs(HH1)

        x_reconstructed = self.idwt((LL2, [torch.stack([LH2, HL2, HH2], dim=2)]))
        x = self.wconv12 (x_reconstructed).permute(0,2,3,1).contiguous()
        return x
...
class MambaIRDWTCAB(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaIRDWTCAB, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
